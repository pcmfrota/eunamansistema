...
//
// É renderizado tanto na sidebar (perto da borda esquerda da tela) quanto na barra
// superior mobile (perto da borda direita) — por isso o painel é posicionado via portal,
// calculado a partir da posição real do botão, em vez de um simples "right-0"/"left-0"
// relativo: um valor fixo funciona bem numa das duas posições e corta o painel na outra.
export default function NotificacoesBell() {
  ...
  // Recalcula a posição sempre que o painel abre (e ao rolar/redimensionar) — o botão pode
  // estar na sidebar ou na barra mobile, então não dá pra assumir uma posição fixa.
  useEffect(() => {
    if (!open) return
    const update = () => {
  ...
  const panel = open && anchorRect && mounted && typeof document !== 'undefined' && (() => {
    const left = Math.min(
    ...
    return createPortal(
      <div
        ...
        className="max-h-96 overflow-y-auto bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl z-[200]"
      >
