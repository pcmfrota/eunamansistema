...
const OSSchema = z.object({
  ...
  status: z.enum(['Aberta', 'Programado', 'Fechada', 'Cancelada', 'Concluída']).default('Aberta'),
  ...
  foto_horimetro: z.string().optional().nullable(),
  foto_km: z.string().optional().nullable(),
  ...
  created_by_nome: z.string().optional().nullable(),
    ...
    for (const row of rows) {
      ...
      try {
        const raw = {
          ...
          status: (() => { const s = String(this.getVal(row, ['status', 'Status', 'Situação', 'situacao', 'Estado', 'STATUS']) || 'Aberta').trim().toLowerCase(); const sm: Record<string, string> = {'fechada':'Fechada','fechado':'Fechada','concluida':'Fechada','concluído':'Fechada','aberta':'Aberta','aberto':'Aberta','em aberto':'Aberta','em andamento':'Aberta','andamento':'Aberta','programado':'Programado','programada':'Programado','agendado':'Programado','agendada':'Programado','cancelada':'Cancelada','cancelado':'Cancelada'}; return sm[s] || 'Aberta'; })(),
          ...
          horimetro: this.parseFloatSafe(this.getVal(row, ['horimetro', 'Horímetro', 'Hori', 'HR', 'HRS'])),
          km: this.parseFloatSafe(this.getVal(row, ['km', 'KM', 'KM/H', 'Hodometro', 'Hodômetro'])),
