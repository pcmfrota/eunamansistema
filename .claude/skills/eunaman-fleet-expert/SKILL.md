...
* [Notas Reais de Funcionamento](./WORKING_NOTES.md) - **Leia primeiro.** Fatos confirmados na prática (não aspiracionais) sobre arquitetura de Server Actions, convenção real de migrations, ausência de tipos gerados, build tolerante a erros, como testar sem navegador, lições de segurança já aprendidas, calendário Suzano e a integração de IA (Gemini).
...
4. **Altere o Banco de Dados via SQL rastreável**: para tabelas/módulos novos e mudanças estruturais grandes, use uma migration em `/supabase/migrations`. Para pedidos pontuais (adicionar uma coluna, ajustar um valor), o padrão real deste projeto é mais leve: um arquivo solto na raiz (`add-x.sql`/`fix-x.sql`) com instrução para o usuário rodar manualmente no SQL Editor do Supabase — ver [Notas Reais de Funcionamento](./WORKING_NOTES.md).
