...
6. **Atualização de Tipos**: Este projeto **não usa tipos gerados automaticamente pelo Supabase CLI** (não existe `database.types.ts`). Os tipos são escritos à mão em `src/models/<modulo>.ts` — após criar/alterar uma tabela, atualize (ou crie) a interface correspondente manualmente lá.

> Para o que realmente acontece na prática neste projeto (nem toda mudança de schema vira uma migration formal versionada), veja [Notas Reais de Funcionamento](./WORKING_NOTES.md).
